...
def auditing_function(data,model=0):
    ...
    for n in range(0, len(data)):
      out_temp = data[n] + q*2*(np.ones(1)-np.array(para).transpose() * data[n]) * np.array(para)
...
noise_data = auditing_function(auditing_data, model = target_model)
...
audtting_result=np.array(audtting_result[0:55])
